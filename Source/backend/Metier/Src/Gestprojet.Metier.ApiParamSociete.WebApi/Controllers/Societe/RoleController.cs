...
namespace Gestprojet.Metier.ApiParamSociete.WebApi.Controllers.Societe
{
    ...
    public class RoleController : ControllerBase
    {
        ...
        public async Task<IActionResult> AjouterOuModifier([FromBody] RoleCore entity)
        {
            if (entity == null) return BadRequest("Données Role invalides");
            var result = await _roleBusiness.AjouterOuModifierAsync(entity);
            return result.Success ? Ok(result.Message) : BadRequest(result.Message);
        }

        [HttpPost("ajouter")]
        public async Task<IActionResult> Ajouter([FromBody] RoleCore entity)
        {
            if (entity == null) return BadRequest("Données Role invalides");
        ...
        [HttpPut("modifier")]
        public async Task<IActionResult> Modifier([FromBody] RoleCore entity)
        {
            if (entity == null) return BadRequest("Données Role invalides");
            var result = await _roleBusiness.AjouterOuModifierAsync(entity);
            return result.Success ? Ok(result.Message) : BadRequest(result.Message);
        ...
        public async Task<IActionResult> GetAll()
            => Ok(await _roleBusiness.ListeAsync());
        ...
        public async Task<IActionResult> Obtenir(string id)
        {
            ...
            var r = await _roleBusiness.ObtenirAsync(id);
            return r == null ? NotFound("Role introuvable") : Ok(r);
        }
        ...
        public async Task<IActionResult> ListeParCritere([FromBody] ConditionRecherche critere)
        {
            ...
            return Ok(await _roleBusiness.ListeParCritereAsync(critere));
        }
        ...
        {
            if (string.IsNullOrWhiteSpace(id)) return BadRequest("Id requis");
            var result = await _roleBusiness.SupprimerAsync(id);
            return result.Success ? Ok(result.Message) : BadRequest(result.Message);
        }
        ...
        public async Task<IActionResult> SupprimerParCondition([FromBody] ConditionRecherche critere)
        {
            ...
            var result = await _roleBusiness.SupprimerParConditionAsync(critere);
            return result.Success ? Ok(result.Message) : BadRequest(result.Message);
        }

        [HttpGet("ListeParPage")]
        public async Task<IActionResult> ListeParPage([FromQuery] int pageNumero = 1, [FromQuery] int pageTaille = 20)
            => Ok(await _roleBusiness.ListeParPageAsync(pageNumero, pageTaille));
        ...
        {
            if (critere == null) return BadRequest("Critère manquant");
            return Ok(await _roleBusiness.ListeParConditionParPageAsync(critere, pageNumero, pageTaille));
        }
    }
}