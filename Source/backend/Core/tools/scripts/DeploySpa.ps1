...
try 
{
	...
	Write-Host
	Write-Host '<###########################################################################>'
	Write-Host "<#           Déploiement du le SPA $SpaName-$Environment/$Version          #>"
	Write-Host '<###########################################################################>'
	Write-Host
	<###########################################################################>
	<#                          CREATION DES VARIABLES                         #>
	<###########################################################################>
	[bool] $SuccesDeploiement = $True
    ...
    {
        $EnvVariable='-{0}' -f $Environment.ToLower()
    } 
	$SpaVersionDirectory = Join-Path $DirectorySites -ChildPath ("Spa{0}{1}\{2}" -f $SpaName, $EnvVariable, $Version)
    ...
    <###########################################################################>
    $WebConfigPath = Join-Path $SpaVersionDirectory -ChildPath 'web.config'
    ...
    <###########################################################################>
    Write-Host ('Suppression des Fichiers et des Sous-Répertoires dans :  {0}' -f $SpaVersionDirectory) -ForegroundColor Gray;

    try {
        Get-ChildItem -Path $SpaVersionDirectory -Include * | Remove-Item -Recurse -Force -ErrorAction Stop
    ...
    try {
        ...
        Expand-ZipFile -ZipFilePath $ZipPathFileName -DestinationDirectoryPath $SpaVersionDirectory -OverwriteWithoutPrompting
