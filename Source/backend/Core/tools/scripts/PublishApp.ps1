...
{
	$SourceProjectFileName='Ociane.Services.' + $AppName + '.Core'
}
...
$SourceProjectPath = Join-Path $SourcePath ('src/Ociane.Services.' + $AppName + '.Core')
$SourceProjectFilePath = Join-Path $SourceProjectPath ($SourceProjectFileName + '.csproj')

$BuildPath = Join-Path $BinariesPath $AppName
...
$DemandesDestPath = Join-Path $DestPath $AppName
...
dotnet publish -c $BuildMode /p:EnvironmentName=$EnvironmentName -o $BinaryPath -r win-x64 --self-contained false 
...
$ZipFileName = $AppName + '-' + $BuildMode + '-' + $Version + '.zip'
