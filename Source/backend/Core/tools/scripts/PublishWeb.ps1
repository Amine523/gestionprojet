...
$SolutionFolder='\src\' + $WebName

$SourceProjectPath = Join-Path $SourcePath $SolutionFolder
$SourceProjectFilePath = Join-Path $SourceProjectPath ($SourceProjectFileName + '.csproj')

$BuildPath = Join-Path $BinariesPath $WebName
...
$DemandesDestPath = Join-Path $DestPath $WebName
...
$ZipFileName = $WebName + '-' + $BuildMode + '-' + $Version + '.zip'
