...
{
	$SourceProjectFileName='CliniPro.Core.' + $ApiName + '.WebApi'
}

# Extract the project name (last part of the path) for DLL name
$ProjectName = Split-Path $SourceProjectFileName -Leaf
$WebApiDllName=$ApiName + '.dll'
...
$SourceProjectPath = Join-Path $SourcePath ($SourceProjectFileName)
$SourceProjectFilePath = Join-Path $SourceProjectPath ($ProjectName + '.csproj')
...
dotnet publish -c $BuildMode /p:EnvironmentName=$EnvironmentName -o $BinaryPath --self-contained false 
