...
#Write-Host ("$ZipPathFileName $Environment $DirectorySites $ApiName $Version") -ForegroundColor Magenta
Clear-Host
...
try 
{
    ...
    Write-Host
    Write-Host '<#########################################################>'
    Write-Host "<#           Déploiement du l'API $ApiName-$Environment-$Version        #>"
    Write-Host '<#########################################################>'
    Write-Host
    <###########################################################################>
	...
	$ApiVersionDirectory = $DirectorySites

    <###########################################################################>
    <#                 SUPPRESSION DU WEB.CONFIG du Web.config                 #>
    <#                  POUR LE RECYCLAGE DU POOL APPLICATIF                   #>
    <###########################################################################>
    $WebConfigPath = Join-Path $ApiVersionDirectory -ChildPath 'web.config'
	...
	<###########################################################################>
    <#                   SUPPRESSION DES WEB.CONFIG INUTILES                   #>
    <#               (QUI NE CORRESPONDENT PAS A L'ENVIRONNEMENT)              #>
    <###########################################################################>
    Write-Host ('Suppression des Web.Config Hors Environnement') -ForegroundColor Gray
	
	try {
		[String]$EnvironmentString = EnvironmentToString($Environment)
		# Si il existe un web.[Environnement].config => Suppression des autres et renommage en web.config
		 If ((Test-Path "$ApiVersionDirectory\web.$EnvironmentString.config")) { 
		   Remove-item $ApiVersionDirectory'\web.config' 
           Remove-item $ApiVersionDirectory'\web.*.config' -exclude *.$EnvironmentString.config -Force
		   Move-Item -Path "$ApiVersionDirectory\web.$EnvironmentString.config" -Destination $ApiVersionDirectory'\web.config'
        }
	}
	catch {
		throw "[error] - Suppression des Web.Config'  - $_"
	}
