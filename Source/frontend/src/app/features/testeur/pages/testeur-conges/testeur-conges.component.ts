...
export class TesteurCongesComponent implements OnInit {
  ...
  soumettreDemande() {
    if (this.nouvelleDemande.typePointageId === 'HALFDAY' || this.nouvelleDemande.typePointageId === 'AUTORISATION') {
      ...
      this.nouvelleDemande.dateFin = this.nouvelleDemande.dateDebut;
    } else {
    ...
    const user = this.api.getCurrentUser();
    const uid = user.id || user.utilisateurId;
    const sid = user.societeId || user.SocieteId || '';
    ...
    this.api.createDemandeCongeReal(dto).subscribe({
      next: (res: any) => {
        ...
        if (this.nouvelleDemande.typePointageId === 'MALADIE' && this.justificatifFile && demandeId) {
          ...
          this.snackBar.open('Votre demande a été envoyée', 'Fermer', { duration: 4000 });
        }
