    ...
    <%@ Import Namespace="NuGet.Server" %>
        <%@ Import Namespace="NuGet.Server.App_Start" %>
            <%@ Import Namespace="NuGet.Server.Infrastructure" %>
                ...
                <body>
                    <div>
                        <h2>You are running NuGet.Server v<%= typeof(NuGetODataConfig).Assembly.GetName().Version %>
                        </h2>
                        <p>
                            Click <a href="<%= VirtualPathUtility.ToAbsolute(" ~/nuget/Packages") %>">here</a> to view
                            your packages.
                        </p>
                        <fieldset style="width:800px">
                            ...
                            <% if (string.IsNullOrEmpty(ConfigurationManager.AppSettings["apiKey"])) { %>
                                To enable pushing packages to this feed using the <a
                                    href="https://www.nuget.org/downloads">NuGet command line tool</a> (nuget.exe), set
                                the <code>apiKey</code> appSetting in web.config.
                                <% } else { %>
                                    Use the command below to push packages to this feed using the <a
                                        href="https://www.nuget.org/downloads">NuGet command line tool</a> (nuget.exe).
                                    <blockquote>
                        ...
                        <% if (Request.IsLocal ||
                            ServiceResolver.Current.Resolve<NuGet.Server.Core.Infrastructure.ISettingsProvider>
                            ...
                            <fieldset style="width:800px">
                                ...
                                Click <a href="<%= VirtualPathUtility.ToAbsolute(" ~/nuget/clear-cache") %>">here</a> to
